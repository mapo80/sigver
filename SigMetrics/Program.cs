...
csv.Add("Method,CountGen,MeanGen,StdGen,CountFor,MeanFor,StdFor,AUC,EER,Prec1,Prec5,LogLoss,ECE");
...
var csvPath = Path.Combine(basePath, "docs", "metrics.csv");
...
void Evaluate(string name, List<double> gen, List<double> forg)
{
    ...
    csv.Add(string.Join(',', new string[]
    {
        name,
        gen.Count.ToString(),
        gStats.mean.ToString("F4"),
        gStats.std.ToString("F4"),
        forg.Count.ToString(),
        fStats.mean.ToString("F4"),
        fStats.std.ToString("F4"),
        auc.ToString("F4"),
        eer.ToString("F4"),
        prec1.ToString("F4"),
        prec5.ToString("F4"),
